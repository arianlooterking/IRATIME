...
export default function createHandler(eventType) {
  return async function handler(req, res) {
    ...
    try {
      const dev = await p.query(
        'SELECT d.*, e.id as employee_id FROM devices d JOIN employees e ON e.id = d.employee_id WHERE device_uuid=$1',
        [device_uuid]
      );
      ...
      const ev = await p.query(
        'INSERT INTO events (employee_id, device_id, type, source, public_ip) VALUES ($1,$2,$3,$4,$5) RETURNING *',
        [dev.rows[0].employee_id, dev.rows[0].id, eventType, source || 'manual', ip]
      );
      const day = todayYMD();
      const evts = await p.query(
        'SELECT * FROM events WHERE employee_id=$1 AND created_at::date = $2::date ORDER BY created_at ASC',
        [dev.rows[0].employee_id, day]
      );
      let work = 0,
        brk = 0,
        state = 'out',
      ...
      await p.query(
        `INSERT INTO timesheets (employee_id,date,work_minutes,break_minutes,overtime_minutes,details) VALUES ($1,$2::date,$3,$4,$5,'[]'::jsonb)
